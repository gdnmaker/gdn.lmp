(function () {
    ...
    function changeMovieTypeLabels() {
        ...
        function addLabel(card) {
            ...
            if (!isTV) {
                ...
                else if ($(card).find('.card__type, .card__temp').text().match(/(сезон|серія|епізод|ТБ|TV)/i)) isTV = true;
            }
    ...
    function initPlugin() {
        Lampa.SettingsApi.addParam({ component: "interface", param: { name: "season_and_episode", type: "trigger", default: true, }, field: { name: "Відображення стану серіала (сезон/серія)", }, onRender: function (element) { setTimeout(function () { $("div[data-name='season_and_episode']").insertAfter("div[data-name='card_interface_reactions']"); }, 0); }, });
